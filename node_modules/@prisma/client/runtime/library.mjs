...
${er(e)}`}var Wm=L("prisma:client:engines:resolveEnginePath"),Jm=()=>new RegExp("runtime[\\\\/]library\\.m?js$");async function ol(e,r){let t={binary:process.env.PRISMA_QUERY_ENGINE_BINARY,library:process.env.PRISMA_QUERY_ENGINE_LIBRARY}[e]??r.prismaPath;if(t!==void 0)return t;let{enginePath:n,searchedLocations:i}=await Hm(e,r);if(Wm("enginePath",n),n!==void 0&&e==="binary"&&di(n),n!==void 0)return r.prismaPath=n;let o=await nr(),s=r.generator?.binaryTargets??[],a=s.some(d=>d.native),l=!s.some(d=>d.value===o),u=__filename.match(Jm())===null,c={searchedLocations:i,generatorBinaryTargets:s,generator:r.generator,runtimeBinaryTarget:o,queryEngineName:sl(e,o),expectedLocation:Ln.relative(process.cwd(),r.dirname),errorStack:new Error().stack},p;throw a&&l?p=nl(c):l?p=rl(c):u?p=tl(c):p=il(c),new T(p,r.clientVersion)}async function Hm(e,r){let t=await nr(),n=[],i=[r.dirname,Ln.resolve(__dirname,".."),r.generator?.output?.value??__dirname,Ln.resolve(__dirname,"../../../.prisma/client"),"/tmp/prisma-engines",r.cwd];__filename.includes("resolveEnginePath")&&i.push(ls());for(let o of i){let s=sl(e,t),a=Ln.join(o,s);if(n.push(o),Qm.existsSync(a))return{enginePath:a,searchedLocations:n}}return{enginePath:void 0,searchedLocations:n}}function sl(e,r){return e==="library"?$t(r,"fs"):`query-engine-${r}${r==="windows"?".exe":""}`}var ro=se(wi());function al(e){return e?e.replace(/".*"/g,'"X"').replace(/[\s:\[]([+-]?([0-9]*[.])?[0-9]+)/g,r=>`${r[0]}5`):""}function ll(e){return e.split(`
